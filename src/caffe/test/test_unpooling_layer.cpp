...
UnpoolingLayerTest()
	...
	void TestForwardSquare(){
		...
		UnpoolingParameter* unpool_param = layer_param.mutable_unpooling_param();
		unpool_param->set_kernel_size(2);
		const int num = 2;
		...
		*/
        ...
        void TestForwardRectHigh(){
            LayerParameter layer_param;
            UnpoolingParameter* unpool_param = layer_param.mutable_unpooling_param();
            unpool_param->set_kernel_h(3);
            unpool_param->set_kernel_w(2);
            const int num = 2;
            const int channels = 2;
